...
And there's a deliberate meta-lesson in choosing a privacy policy as the demo: **this telemetry is itself a small data-collection system, so it practices what the document preaches.** What's logged: tool names, tool arguments, and the LLM's summarized intent. What's not: no account identity, no IP addresses in the analytics logs, no raw conversation text — the assistant sends a summary, not the chat. The flip side — what it means to *hold* intent data at all — gets its own section, because it's the part most demos skip.

### 3.4 The privacy implications of knowing user intent

Everything that makes intent data useful makes it sensitive. If you teach one critical-thinking moment in this class, make it this one:

- **Questions reveal circumstances.** An access log says someone opened the policy. An intent record says *why*: "user wants to delete a chat containing medical information," "parent worried their teen is in crisis," "employee asking whether their boss can read their chats." People asking how to delete sensitive data have just told you they have sensitive data. `user_goal` is closer to a confession than a page view.
- **Free-text fields collect more than you designed.** There's no schema field for "medical condition," but the LLM writes the summaries, and users overshare — so identifying or intimate details can arrive in `user_goal` and `summary` anyway. This demo mitigates at the prompt level: the `log_activity` tool description instructs the model to describe intent *generically* and omit names, contact details, and identifying specifics. That helps; it is not a guarantee.
- **"Anonymous" wears thin at low volume.** The logs carry no identity, but on a small deployment — one classroom, one school, one company — a timestamp plus a user type plus a specific concern can be enough for someone with context to guess who asked. Aggregate dashboards de-risk this; the raw `activities.jsonl` and the "Recent Activity Detail" table do not.
- **The consent gap is structural to MCP.** The human consented to their AI assistant. They likely have no idea their question was summarized and shipped to a third-party server's logs — they never see your docs page disclosure before asking. The assistant's platform decides what to send; you decide what to keep. Nobody in that chain got a clear yes from the user. There's no clean fix today; the honest moves are minimal collection, prominent disclosure, and short retention.
- **The dashboard publishes what the logs collect.** In this demo `/reporting/` is deliberately open so the class can watch the loop close — which means every self-reported user goal is on the public internet. For any real deployment, put the dashboard behind auth (Caddy `basicauth` is two lines) before you put the server behind a domain.
- **If you run this, you're a data processor now.** Operating an intent-collecting MCP server makes you the holder of (potentially) personal data, with everything that implies under GDPR-style and U.S. state laws: retention limits, access control, the ability to delete. The demo gives you the mechanics — local-only NDJSON files, log rotation, no third-party analytics — but the obligations are yours.

The recursive joke of this lesson is the point: we built a tool for asking *"what does this company know about me?"* — and instrumenting it created exactly the same question one level down. Whoever publishes an MCP server is somebody else's data controller.
